...
class FavouriteViewModelImpl : ViewModel(), FavouriteViewModel {
    ...
    private fun initWords() {
        ...
        when (repository.getCurrentLanguage()) {
            false -> {
                val data = repository.getAllFavouriteEnglishWords()
                allWords.value = data
                noResult.value = data.count > 0
            }
            else -> {
                val data = repository.getAllFavouriteUzbekWords()
                allWords.value = data
                noResult.value = data.count > 0
            }
        }
    ...
    override fun allWordsByQuery(query: String?) {
        query?.let {
            when (repository.getCurrentLanguage()) {
                false -> {
                    val data = repository.getAllFavouriteEnglishWordsByQuery("%$it%")
                    allWordsByQuery.value = data
                    noResult.value = data.count > 0
                }
                else -> {
                    val data = repository.getAllFavouriteUzbekWordsByQuery("%$it%")
                    allWordsByQuery.value = data
                    noResult.value = data.count > 0
                }
            }
